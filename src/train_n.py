...
def train_segmenter(
    ...
    Args:
      ...
      freeze_bn (bool) : whether to keep BN params(激活函数) intact
    ...
    for i, sample in enumerate(train_loader,0):
        ...
        #soft_output = nn.functional.log_softmax(output)
...
def validate(
    ...
    with torch.no_grad():
        for i, sample in enumerate(val_loader):
            ...
            output = cv2.resize(output, target.size()[1:][::-1], interpolation=cv2.INTER_CUBIC)
            cmap = np.load('../utils/cmap.npy')
            output = cmap[output.argmax(axis=2).astype(np.uint8)]
            '''
            '''
...
def main():
   ...
   # if torch.cuda.is_available():
    ...
    ## Criterion ##
    segm_crit = nn.NLLLoss2d(ignore_index=args.ignore_label)
    #segm_crit = nn.CrossEntropyLoss()
    ## Saver ##
